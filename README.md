...
| `send --chain c --to <addr> --amount N [--asset usdc\|native] [--network n]` | Raw transfer to an arbitrary address. Default `--asset usdc` (ERC20 / SPL); `--asset native` sends gas (ETH / TEMPO / SOL). Requires native gas in the signer wallet for the on-chain write. No merchant, no 402 handshake. EVM transfers confirm the receipt (a reverted tx returns `transfer_reverted`, never a hash for a tx that moved nothing). On Tempo the fee is paid in the stablecoin, so a full-balance USDC send reverts; leave a little `--amount` headroom. |
