...
| `src/commands/send.ts` | `send --chain <chain> --to <addr> --amount <n> [--asset usdc\|native]` — raw transfer (no merchant / 402). Default `--asset usdc` (ERC20 transfer on EVM / SPL transferChecked + idempotent ATA on Solana). `--asset native` sends ETH/TEMPO/SOL via viem sendTransaction (EVM) or `getTransferSolInstruction` (Solana). Both flavors require native gas in the signer wallet for the on-chain write. Works on mainnet + testnet (`--network testnet`). |
