...
| `src/commands/send.ts` | `send --chain <chain> --to <addr> --amount <usdc>` — raw USDC transfer (no merchant / 402 handshake). EVM via viem `writeContract(transfer)`, Solana via SPL `transferChecked` + idempotent ATA creation. Requires native gas in the signer wallet. |
