...
describe('passport commands', () => {
  ...
  describe('passportLoginCommand', () => {
    it('mints a session, polls until verified, saves the passport', async () => {
      ...
      expect(verifyUrls).toEqual(['https://www.agentscore.com/verify?session=sess_abc']);
        ...
        if (target.endsWith('/v1/sessions/public') && init?.method === 'POST') {
          return new Response(
            JSON.stringify({
              ...
              poll_secret: 'poll_x',
              verify_url: 'https://www.agentscore.com/verify?session=sess_deny',
        ...
        if (target.endsWith('/v1/sessions/public') && init?.method === 'POST') {
          return new Response(
            JSON.stringify({
              ...
              poll_secret: 'poll_x',
              verify_url: 'https://www.agentscore.com/verify?session=sess_timeout',
